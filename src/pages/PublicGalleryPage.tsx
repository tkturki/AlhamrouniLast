...
const PublicGalleryPage: React.FC = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      if (storedItems) {
        ...
        setItems(parsedItems.filter((item: JewelryItem) => item.stock_qty > 0));
      }
    } catch (error) {
  ...
  const filteredItems = items.filter(item => {
    ...
    switch (sortBy) {
      ...
      case 'popular':
        return ((b as any).view_count || 0) - ((a as any).view_count || 0);
      default:
  ...
  const toggleFavorite = (itemCode: string) => {
    ...
    if (favorites.includes(itemCode)) {
      newFavorites = favorites.filter(f => f !== itemCode);
    } else {
      newFavorites = [...favorites, itemCode];
    }
  ...
  const handleContactSubmit = () => {
    ...
    requests.push({
      ...
      item_name: selectedItem?.model_name,
  ...
  const shareItem = (item: JewelryItem) => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        text: `تفضلوا بزيارة: ${item.model_name}`,
        ...
        <div className="container mx-auto">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2 bg-white/20 backdrop-blur rounded-xl px-4 py-2">
              ...
              <span className="text-gray-900 font-bold">
                {localStorage.getItem('gallery_visits') || 0} زائر
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-4 mt-4">
            ...
            <a href="/contact" className="flex items-center gap-2 bg-white/20 backdrop-blur px-4 py-2 rounded-xl text-gray-900 hover:bg-white/30 transition-all">
              ...
              <span className="font-medium">أرسل رسالة</span>
            </a>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <select
              ...
              className="bg-gray-700/80 border border-gray-600 text-white px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
            >
              <option value="all">كل العيارات</option>
              {karats.filter(k => k !== 'all').map(karat => (
                <option key={karat} value={karat}>{getKaratLabel(karat)}</option>
              ))}
            </select>
            ...
            <div className="flex gap-1 mr-auto">
              <button
                ...
                className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-yellow-600 text-white' : 'bg-gray-700/80 text-gray-400'}`}
              >
                <Grid className="w-5 h-5" />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-yellow-600 text-white' : 'bg-gray-700/80 text-gray-400'}`}
              >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredItems.map((item) => (
              <div
                key={item.id}
                className="bg-gray-800/80 backdrop-blur rounded-2xl overflow-hidden border border-gray-700 hover:border-yellow-500/50 transition-all group shadow-xl"
              >
                {/* Image */}
                <div
                  className="relative aspect-square bg-gray-700 cursor-pointer overflow-hidden"
                ...
                >
                  {item.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-yellow-600/20 to-yellow-800/20">
                      <Gem className="w-20 h-20 text-yellow-600/50" />
                    </div>
                  )}

                  {/* Overlay */}
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    <span className="bg-yellow-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2">
                  ...
                  <div className="absolute top-3 left-3 flex flex-col gap-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleFavorite(item.item_code); }}
                      className={`p-2 rounded-full transition-all ${favorites.includes(item.item_code) ? 'bg-red-500 text-white' : 'bg-white/80 text-gray-700 hover:bg-red-500 hover:text-white'}`}
                    >
                      <Heart className={`w-5 h-5 ${favorites.includes(item.item_code) ? 'fill-current' : ''}`} />
                    </button>
                    <button
                      onClick={(e) => { e.stopPropagation(); shareItem(item); }}
                      className="p-2 rounded-full bg-white/80 text-gray-700 hover:bg-blue-500 hover:text-white transition-all"
                    >
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="bg-yellow-600/20 text-yellow-400 px-2 py-1 rounded text-sm whitespace-nowrap mr-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-sm mb-3">
                    ...
                    <div className="bg-gray-700/50 rounded-lg p-2 text-center">
                      <p className="text-gray-400 text-xs">المخزون</p>
                      <p className="font-bold text-white">{item.stock_qty}</p>
                    </div>
                  </div>

                  <button
                    onClick={() => handleOpenItem(item)}
                    className="w-full bg-gradient-to-r from-yellow-600 to-yellow-500 hover:from-yellow-700 hover:to-yellow-600 text-gray-900 font-bold py-2 rounded-lg flex items-center justify-center gap-2 transition-all"
                  >
          ...
          <div className="space-y-4">
            {filteredItems.map((item) => (
              <div
                key={item.id}
                className="bg-gray-800/80 backdrop-blur rounded-2xl overflow-hidden border border-gray-700 hover:border-yellow-500/50 transition-all flex cursor-pointer"
                onClick={() => handleOpenItem(item)}
              >
                {/* Image */}
                <div className="w-40 h-40 bg-gray-700 flex-shrink-0 relative">
                  {item.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-yellow-600/20 to-yellow-800/20">
                      <Gem className="w-12 h-12 text-yellow-600/50" />
                    </div>
                  )}
                  {(item as any).view_count > 10 && (
                ...
                <div className="flex-1 p-4 flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-4 text-sm text-gray-400">
                      ...
                      <span>المخزون: <span className="text-white font-medium">{item.stock_qty}</span></span>
                    </div>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); handleOpenItem(item); }}
                    className="bg-gradient-to-r from-yellow-600 to-yellow-500 hover:from-yellow-700 hover:to-yellow-600 text-gray-900 font-bold py-3 px-6 rounded-xl flex items-center gap-2 mr-4 transition-all"
                  >
            ...
            <div className="relative bg-gray-900 aspect-square md:aspect-video">
              {itemImages[currentImageIndex]?.url ? (
                itemImages[currentImageIndex]?.type === 'video' ? (
                  <video
                    src={itemImages[currentImageIndex].url}
                    controls
                  ...
                  <img
                    src={itemImages[currentImageIndex].url}
                    alt={selectedItem.model_name}
                    className="w-full h-full object-contain"
                  />
                )
              ) : (
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-yellow-600/20 to-yellow-800/20">
                  <Gem className="w-32 h-32 text-yellow-600/50" />
                </div>
              )}
              ...
              {itemImages.length > 1 && (
                <>
                  <button
                    onClick={() => setCurrentImageIndex(prev => (prev === 0 ? itemImages.length - 1 : prev - 1))}
                  ...
                  <button
                    onClick={() => setCurrentImageIndex(prev => (prev === itemImages.length - 1 ? 0 : prev + 1))}
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                <button
                  onClick={() => toggleFavorite(selectedItem.item_code)}
                  className={`p-2 rounded-full transition-all ${favorites.includes(selectedItem.item_code) ? 'bg-red-500 text-white' : 'bg-black/50 text-white'}`}
                >
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {itemImages.map((img, idx) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-lg overflow-hidden border-2 transition-all ${idx === currentImageIndex ? 'border-yellow-500' : 'border-transparent opacity-70 hover:opacity-100'}`}
                    >
                      {img.url ? (
                        img.type === 'video' ? (
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="bg-yellow-600 text-white px-4 py-2 rounded-full font-bold">
                  {getKaratLabel(selectedItem.karat)}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-gray-700/50 rounded-xl p-4 text-center">
                  ...
                  <p className="text-2xl font-bold text-white flex items-center justify-center gap-1">
                    <Eye className="w-5 h-5" /> {(selectedItem as any).view_count || 0}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-yellow-600 to-yellow-500 hover:from-yellow-700 hover:to-yellow-600 text-gray-900 font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all"
                >
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-gray-400 text-sm mb-2">رسالة (اختياري)</label>
                <textarea
                  value={contactForm.message}
                  onChange={(e) => setContactForm({ ...contactForm, message: e.target.value })}
                  className="w-full bg-gray-700 border border-gray-600 text-white px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none h-24"
      ...
      <footer className="bg-gray-900 border-t border-gray-800 py-8 mt-12">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex justify-center gap-6 mb-6">
            <a href="tel:+218912133218" className="text-gray-400 hover:text-yellow-500 flex items-center gap-2">
              <Phone className="w-5 h-5" />
            ...
            <a href="/contact" className="text-gray-400 hover:text-yellow-500 flex items-center gap-2">
              <MessageCircle className="w-5 h-5" />
              <span>أرسل رسالة</span>
            </a>
          </div>
          <p className="text-gray-600 text-sm">
            جميع الحقوق محفوظة © {new Date().getFullYear()} مجوهرات الحمروني
          </p>
        </div>
      </footer>
    </div>
